<template>
	<div id="neworder">
      ...
      <!-- header end -->
      	<div class="myorders">
      		<ul>
      			<li v-for="(item,index) in $store.state.community.orders">
      				<span>{{item.name}}</span>
      				<span><span>￥{{item.sum}}</span>{{item.date.toLocaleDateString()}}</span>
      			</li>
      		</ul>
      	</div>
	</div>
		...
		.myorders{
			...
			ul{
				...
				li{
					...
					border-bottom:.5px solid rgba(255,125,125,.5);
					span{
						display: block;
					}
				}
